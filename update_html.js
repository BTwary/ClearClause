...
analyzeBtn.addEventListener('click', async ()=>{
  ...
  // If the PDF pipeline already classified this document (with a confidence score),
  // reuse that instead of making the worker re-classify from scratch.
  let documentTypeHint = null;
  const pdfDoc = window.__trothixDocument;
  if (pdfDoc && pdfDoc.typeConfidence >= 0.85) {
    if (pdfDoc.documentType === 'Non-Disclosure Agreement') documentTypeHint = 'NDA';
    else if (pdfDoc.documentType === 'Rental Agreement') documentTypeHint = 'LEASE';
  }
  ...
  parserWorker.onmessage = (e) => {
    ...
    if (result.isFullyLocal) {
      ...
      const d = {
        ...
        topPoints: result.flags.slice(0, 3).map(f => f.message),
