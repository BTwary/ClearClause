...
const ENABLE_ANALYTICS =
  ...
  process.env.TROTHIX_ENABLE_ANALYTICS?.toLowerCase() === "true";
...
export default async function handler(req, res) {
  ...
  if (mode === "synthesize") {
    if (!findings || typeof findings !== "object") {
      await safeRecordEvent("error", { reason: "missing_findings" });
    ...
    if (mode === "chunk") {
      if (documentText.length > 16000) {
        await safeRecordEvent("error", { reason: "chunk_too_large" });
  ...
  if (!apiKeys.gemini && !apiKeys.groq && !apiKeys.mistral && !apiKeys.openrouter) {
    await safeRecordEvent("error", { reason: "missing_api_key" });
  ...
  try {
    ...
    if (!result || !result.ok) {
      ...
      await safeRecordEvent(status === 429 ? "rate_limited" : "error", status === 429 ? undefined : { reason: "upstream_5xx" });
    ...
    if (!text) {
      ...
      await safeRecordEvent("error", { reason: "empty_response" });
    ...
    try {
      ...
      try {
        ...
        await safeRecordEvent("error", { reason: "json_parse_failed" });
        return res.status(502).json({
          error:
            finishReason === "MAX_TOKENS"
    ...
    if (parsed && parsed.isDocument === false) {
      await safeRecordEvent("not_document");
    } else {
    ...
    await safeRecordEvent("error", { reason: "unexpected" });
